...
각 사용자는 설정 페이지에서 실전투자 활성화 여부를 개별적으로 설정할 수 있습니다.

- **비활성화 (기본값)**: 전략 신호가 발생해도 실제 주문이 실행되지 않으며, API 키와 주문 금액 검증만 수행됩니다.
- **활성화**: 전략 신호에 따라 연결된 Upbit 계좌에서 실제 매수/매도가 자동으로 실행됩니다.

웹 UI의 계정 설정 페이지에서 "실전투자" 토글을 사용하여 언제든지 변경할 수 있습니다.
