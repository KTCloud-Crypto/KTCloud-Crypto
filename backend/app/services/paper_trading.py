...
from app.services.strategy_allocation import allocation_within_limit, portfolio_buy_amount
...
def execute_paper_order(
    ...
    if execution.action == "buy":
        if not allocation_within_limit(db, execution.user_id, "simulated"):
            _fail(execution, "선택된 전략의 투자 비율 합계가 100%를 초과합니다.")
            return
        if position.volume > 0:
            _skip(execution, "기존 모의 포지션을 보유 중이므로 중복 매수 신호를 건너뛰었습니다.")
            return
        cash = Decimal(account.cash_balance)
        total_equity = account_value(db, execution.user_id).total_equity
        amount = portfolio_buy_amount(
