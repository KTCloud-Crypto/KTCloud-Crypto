...
from app.services.strategy_allocation import allocation_within_limit
...
def _prepare_live_execution(db: Session, target: ExecutionTarget) -> PreflightResult:
    ...
    if target.action == "buy":
        if not allocation_within_limit(db, target.user_id, "live"):
            return PreflightResult(
                False,
                None,
                "선택된 전략의 투자 비율 합계가 100%를 초과해 신규 매수를 차단했습니다.",
            )
